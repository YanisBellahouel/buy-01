    ...
    stages {
        ...
        // 🔍 SONARQUBE ANALYSIS
			stage('SonarQube Analysis - User Service') {
				steps {
					dir('microservices/user-service') {
						withSonarQubeEnv('SonarQube') { // Jenkins injecte ici le Token et l'URL
							sh "mvn sonar:sonar -Dsonar.projectKey=sonarbuy"
						}
					}
				}
			}
			...
			stage('Quality Gate') {
				steps {
					// Cette étape utilise maintenant le token configuré globalement pour s'authentifier
